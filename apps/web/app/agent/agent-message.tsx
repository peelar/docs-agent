...
export function AgentMessage({
  ...
  return (
    <Message
      className={message.role === "user" ? "w-fit max-w-2xl" : "max-w-3xl"}
    ...
    case "dynamic-tool":
      return (
        ...
        >
          <ToolHeader
            state={part.state}
            title={part.toolMetadata?.eve?.name ?? part.toolName}
    ...
    <span className="flex max-w-sm items-center gap-3 rounded-md border bg-background/60 p-2 text-sm">
      {isImage ? (
        <Image
          ...
          height={48}
          src={part.url}
          // Eve may return browser-local blob and data URLs that cannot pass
...
function authorizationDescription(part: EveAuthorizationPart): string {
  ...
  if (part.outcome === "authorized") return `${part.displayName} connected.`;
  const reason = part.reason === undefined ? "" : ` (${part.reason})`;
