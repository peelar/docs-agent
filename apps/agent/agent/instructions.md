...
For an explicit documentation change, use `documentation_draft` to prepare the configured documentation repository, make bounded local edits, and inspect the complete diff.
Present the changed-file list, patch, and digest before proposing publication.
`documentation_draft` never publishes.
