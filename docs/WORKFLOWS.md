...
| --- | --- | --- | --- |
| Signal intake | Convert provider context into a docs signal with provenance. | `create_docs_signal`, `list_docs_signals`, `get_docs_signal`, `update_docs_signal_lifecycle`; watched scans also create source evidence. | Inspect or patch docs directly. |
...
| Current-docs verification | Inspect the configured working documentation repository in the sandbox. | Existing scenario workflow in `run_docs_maintenance_scenario`; later a signal-verification wrapper should call the same repository workflow boundary. | Publish or write outside `/workspace/working-docs`. |
...
- `run_docs_maintenance_scenario`: stays as the eval and scenario terminal
  workflow for now. It should not become the general channel tool name. A later
  signal-verification tool should wrap or reuse its repository workflow pieces
  instead of duplicating clone/search/patch/check/diff behavior.
...
3. `create_docs_signal` creates or dedupes the signal.
...
6. Current-docs verification materializes `/workspace/working-docs` and checks
   likely docs pages.
