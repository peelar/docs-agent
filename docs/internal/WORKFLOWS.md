...
idempotent fixed-UTC-hour slot. The watch runtime then claims that reservation
with a bounded lease and starts one Eve turn carrying the exact goal, policy,
and provider-neutral occurrence. Zero-retention handoffs remain in memory for
one direct claim only and are never eligible for durable scanning or retry. The
same Eve minute schedule claims due
`on_schedule` occurrences, closes bounded windows, retries expired handoffs,
flushes digests, and clears expired runtime data; no second scheduler or
workflow engine exists. Ignore is a redacted no-op with no ordinary response.
Provider delivery is a separate source-bound, budgeted, idempotent capability;
frozen digest batches fail as a whole if any member loses authority;
silent turns expose no delivery tool.
...
| Watch dispatch readiness | Revalidate and reserve a bounded handoff before the watch-turn executor. | Current verified provider workspace, canonical setup, active exact effective revision, source/event/retention/claim validation, available approved capabilities, and an idempotent fixed-UTC-hour processing-budget reservation. | Inherit a replacement revision; expose provider payload fields or extra capability; dispatch after pause/deletion/expiry/authorization loss/budget exhaustion; start Eve or create signals, memories, replies, digests, or follow-ups inside readiness. |
| Watch goal execution | Run one exact policy-bound occurrence through composable Eve capabilities and an independent delivery mode. | Slack adapter plus one Eve minute schedule claim durable reservations with capped compare-and-set leases; dynamic goal/instructions/skill and tool resolution re-read claim, revision, source, grants, budgets, and expiry; redacted outcomes record action or no-op; source-bound immediate and digest delivery use stable provider idempotency keys. | Add a workflow engine, persist raw content past approved retention, accept a model target, expose publication, let stale claim tokens mutate state, turn ignore into a tool or ordinary reply, or let a digest retry absorb newly queued items. |
