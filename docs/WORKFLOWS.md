...
| Validation result recording | Give the control plane a durable assurance source without treating console or workflow internals as an API. | Typed shared run/case service, structured Eve reporter callbacks, explicit live-eval versus deterministic-validation kinds, redacted summaries, stable ids, and 30-day bounded cleanup. | Parse CLI output, store prompts, outputs, context, reasoning, event streams, tool payloads, or credentials; run arbitrary commands from the browser; hide persistence failures. |
