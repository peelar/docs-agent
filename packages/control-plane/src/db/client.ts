...
export function resolveDocsAgentDatabaseConfig(
  ...
  const configuredUrlOrDefault = url ?? DEFAULT_LOCAL_DATABASE_URL;
  const authToken = env[DOCS_AGENT_DATABASE_AUTH_TOKEN_ENV]?.trim() || undefined;
  const localFilePath = resolveLocalFilePath(
    configuredUrlOrDefault,
    findWorkspaceRoot(cwd),
  );
  const resolvedUrl = localFilePath === undefined
    ? configuredUrlOrDefault
    : pathToFileURL(localFilePath).href;
