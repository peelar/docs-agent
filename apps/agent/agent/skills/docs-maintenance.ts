...
export default defineDynamic({ events: { "turn.started": async (event, context) => {
  ...
  return defineSkill({
    ...
    markdown: [
      ...
      "- Use `working_repository` validators mode only for optional inspection. When a check is requested, pass its named id directly to `run_validators` instead of substituting status or diff. `run_validators` is atomic read-only inspection: it discovers and persists the current source-bound trusted profile, executes only requested ids from that profile, accepts no command, and does not mutate the repository. Inspect status and the bounded draft diff through the same capability.",
      ...
      "- Keep localized changes inline. For substantial work, keep the originating signal and Eve session, use `docs_work_manage` to start and update the original work, and record a typed `plan` before drafting.",
